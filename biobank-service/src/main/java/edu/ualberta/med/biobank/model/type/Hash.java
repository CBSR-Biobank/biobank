...
public abstract class Hash implements Serializable {
	...
	protected static <T extends Hash> T setBytes(T hash,
			MessageDigest algorithm, File file) throws IOException {
		InputStream is = null;
		try {
	...
	@Embeddable
	public static class Md5Hash extends Hash {
		public static final int BYTES_LENGTH = 16;

		private static final long serialVersionUID = 1L;
		private static final String BYTES_COLUMN_DEFINITION = "BINARY("
				+ BYTES_LENGTH + ")";

		@Override
		@NotNull(message = "{Md5Hash.bytes.NotNull}")
		@Size(min = BYTES_LENGTH, max = BYTES_LENGTH, message = "{Md5Hash.bytes.Length}")
		@Column(name = "MD5_HASH", nullable = false, columnDefinition = BYTES_COLUMN_DEFINITION)
		public byte[] getBytes() {
	...
	@Embeddable
	public static class Sha1Hash extends Hash {
		public static final int BYTES_LENGTH = 20;
		...
		@Size(min = BYTES_LENGTH, max = BYTES_LENGTH, message = "{Sha1Hash.bytes.Length}")
		@Column(name = "SHA1_HASH", nullable = false, columnDefinition = BYTES_COLUMN_DEFINITION)
		public byte[] getBytes() {
			return super.getBytes();
		}
