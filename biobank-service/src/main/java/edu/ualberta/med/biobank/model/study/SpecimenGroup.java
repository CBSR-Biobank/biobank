...
import edu.ualberta.med.biobank.model.AnatomicalSource;
...
/**
 * Ownership, summary, storage, and classification information that applies to an entire group or
 * collection of {@link Specimen}s.
 * <p>
 * The owning {@link Study} of a {@link Specimen} is determined by
 * {@link Specimen#getSpecimenGroup()}.{@link SpecimenGroup#getStudy()}. Therefore, if and when
 * {@link Specimen}s move from one {@link Study} to another, they must actually be moved
 * specifically and carefully from one {@link SpecimenGroup} to another.
 * <p>
...
public class SpecimenGroup
    ...
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "ANATOMICAL_SOURCE_ID", nullable = false)
    ...
    /**
     * @return how the {@link Specimen#amount} of each {@link Specimen} is measured (e.g. in mL, m,
     *         g, mg, etc.).
     */
