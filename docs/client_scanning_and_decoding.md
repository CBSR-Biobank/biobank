...
an image on the right. The settings the user can select are the following:
...
<table>
  ...
  <tr>
    ...
    <td>
	  ...
	  the tubes. However, sometimes, some images may have them on the tops.
    </td>
  </tr>
  <tr>
    ...
    <td>
      ...
      user to change the DPI (*dots per inch*) setting the image will be scanned at. This setting is
...
the software to determine where the first cell, **A1**, is located. Each tube has a **position
label** where each row uses an upper case letter starting at **A**, and each column
uses a number starting at **1**. The fist tube has **A1** as a position label. On a 96 well
micro plate the last tube has **H12** as a position label.

### Decoding image from flatbed scanner

Once an image source and the settings selected, the user can press the **Scan** button to acquire an
...
The scanned image is now displayed in the dialog box. Superimposed, is a grid with the number of
cells that corresponds to the dimensions selected in **Plate dimensions**. Each cell of the grid
represents a well in the plate. The well may or may not have a tube placed in it. If a tube is
present in the well it will be decoded. If a tube is not present then nothing will be decoded. It is
important that each cell contains the entire barcode so that the barcode is decoded properly.
...
Once the cells are aligned correctly the user can press the **Decode** button to process each
barcode. Decoding the barcodes will take a few seconds. The image shown below shows the dialog box
after decoding has completed.

![Decoded plate 1](images/decode_image_dialog_plate1_decoded.png?raw=true "Decoded plate 1")

Due to ice build up or dust on the flatbed scanner it is possible that not all the barcodes will be
decoded. Each cell that was decoded has a green check box displayed in the top left. The text below
the image will be updated to state the total number of tubes decoded. Hovering over a cell now
displays the position label along with the text contained on the barcode.
...
1. Press the **Scan** button and then the **Decode** button to start from scratch. This may be done
   if the flatbed was cleaned if the image was dirty. In this case the history of what was decoded is
   lost.
...
1. Resize the grid and press the **Decode** button. Sometimes, missed cells will be decoded.
...
Tube **A1** is at the top left and tube **H1** is at the top left (as shown by where the mouse is
hovering).
...
_Note that in this image the barcodes are present on the tops of the tubes and the setting selected
for **Barcode positions** is **Tube Tops**.
