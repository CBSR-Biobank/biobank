    ...
    public void paintContent(VControl control, Event e) {
        if (control.hasStyle(SWT.SEPARATOR)) {
            ...
            if (control.hasStyle(SWT.HORIZONTAL)) {
                ...
                e.gc.drawLine(bounds.x - 1, bounds.y, bounds.x + bounds.width,
                    bounds.y);
                e.gc.setForeground(display
                    .getSystemColor(SWT.COLOR_WIDGET_LIGHT_SHADOW));
                e.gc.drawLine(bounds.x + 1, bounds.y + 1, bounds.x
                    + bounds.width - 2, bounds.y + 1);
            } else {
                e.gc.setForeground(display
                    .getSystemColor(SWT.COLOR_WIDGET_NORMAL_SHADOW));
                e.gc.drawLine(bounds.x, bounds.y - 1, bounds.x, bounds.y
                    + bounds.height);
                e.gc.setForeground(display
                    .getSystemColor(SWT.COLOR_WIDGET_LIGHT_SHADOW));
                e.gc.drawLine(bounds.x + 1, bounds.y + 1, bounds.x + 1,
