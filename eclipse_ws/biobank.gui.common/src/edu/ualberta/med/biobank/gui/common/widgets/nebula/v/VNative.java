...
public class VNative<T extends Control> extends VControl {
    ...
    public static <T extends Control> VNative<T> create(Class<T> type,
        VPanel parent, int style) {
        try {
            Constructor<T> constructor = type.getConstructor(Composite.class,
                int.class);
    ...
    public String getText() {
        try {
            return (String) control.getClass().getMethod("getText") //$NON-NLS-1$
                .invoke(control);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void setControl(T control) {
        if (this.control != null) {
            throw new UnsupportedOperationException(
                "Can only set a control once"); //$NON-NLS-1$
        }
    ...
    public void setText(String text) {
        try {
            control.getClass().getMethod("setText", String.class) //$NON-NLS-1$
                .invoke(control, text);
        } catch (Exception e) {
            e.printStackTrace();
            super.setText(text);
