        ...
        public void handleEvent(Event event) {
            ...
            case SWT.Traverse:
                ...
                if (SWT.TRAVERSE_TAB_NEXT == event.detail
                    || SWT.TRAVERSE_TAB_PREVIOUS == event.detail) {
                    event.doit = true;
            ...
            case SWT.MouseDown:
                ...
                if (activeControl != null
                    && activeControl.setState(VControl.STATE_MOUSE_DOWN, true)) {
                    activeControl.redraw();
                }
                break;
            case SWT.MouseMove:
                if (panels.containsKey(event.widget)) {
                    VControl vcontrol = panels.get(event.widget).getControl(
                        event.x, event.y, true);
                    if (vcontrol != activeControl
                        && (vcontrol == null || vcontrol.isEnabled())) {
                        activate(vcontrol);
                    }
                } else if (activeControl != null
                    && event.widget != activeControl.getControl()) {
            ...
            case SWT.MouseUp:
                ...
                mouseDown = null;
                if (activeControl != null
                    && activeControl.setState(VControl.STATE_MOUSE_DOWN, false)) {
                    activeControl.redraw();
                }
                break;
            }
    ...
    public void widgetDisposed(DisposeEvent e) {
        ...
        if (tracker.panels != null && tracker.panels.containsKey(e.widget)) {
            ...
            if (tracker.panels.isEmpty()) {
                ...
                Display.getDefault()
                    .removeFilter(SWT.MouseMove, tracker.filter);
                Display.getDefault()
                    .removeFilter(SWT.MouseDown, tracker.filter);
