...
import edu.ualberta.med.biobank.gui.common.widgets.nebula.IControlPainter;
...
public abstract class VControl {
    ...
    void handleEvent(Event event) {
        ...
        if (listeners.containsKey(event.type)) {
            Listener[] la = listeners.get(event.type).toArray(
                new Listener[listeners.get(event.type).size()]);
    ...
    void attachListeners(boolean keyListeners) {
        ...
        // System.out.println("this: " + this + ", focusControl: " +
    ...
    public Rectangle getClientArea() {
        return new Rectangle(bounds.x + marginLeft, bounds.y + marginTop,
            bounds.width - (marginLeft + marginRight), bounds.height
    ...
    private boolean include(boolean key, int type) {
        ...
        if (key
            && (type == SWT.KeyDown || type == SWT.KeyUp || type == SWT.Traverse)) {
            return true;
        }
        if (!key
            && !(type == SWT.KeyDown || type == SWT.KeyUp || type == SWT.Traverse)) {
    ...
    public boolean isEnabled() {
        return getEnabled()
            && ((parent != null) ? parent.isEnabled() : composite.isEnabled());
    }
    ...
    public final void paintControl(Event e) {
        if (painter != null && bounds.intersects(e.x, e.y, e.width, e.height)
            ...
            fullY = bounds.y - 1;
            if (parent != null) {
                fullW = Math.min(parent.bounds.x + parent.bounds.width
                    - bounds.x, bounds.x + bounds.width) + 1;
                fullH = Math.min(parent.bounds.y + parent.bounds.height
