...
public class VGridLayout extends VLayout {
    ...
    private void doLayout(VPanel parent, VControl[] children) {
        ...
        int xconsumed = marginLeft + (2 * marginWidth) + marginRight
            + (horizontalSpacing * (widths.length - 1));
        int yconsumed = marginTop + (2 * marginHeight) + marginBottom
            + (verticalSpacing * (heights.length - 1));
        int xgrabbers = 0;
        int ygrabbers = 0;
        ...
        int xslop = makeColumnsEqualWidth ? (int) Math.ceil((size.x - border
            - xconsumed - (grabWidth * xgrabbers)) / 2) + 1 : 0;

        int xoffset = (parent instanceof VPanel) ? ((VPanel) parent)
            .getBounds().x : 0;
        int yoffset = (parent instanceof VPanel) ? ((VPanel) parent)
            .getBounds().y : 0;
        ...
        for (VControl child : children) {
            ...
            GridData data = child.getLayoutData();
            cellWidth = 0;
            for (int i = 0; i < data.horizontalSpan
                && (col + i) < widths.length; i++) {
                if ((col + i) == 0) {
                    cellWidth += xslop;
                } else if ((col + i) == widths.length - 1) {
                    cellWidth = size.x - border - cellX - marginWidth
                        - marginRight - 1;
                    break;
                }
                cellWidth += (widths[col + i] == -1) ? grabWidth : widths[col
                    + i];
            }
            cellHeight = 0;
            for (int i = 0; i < data.verticalSpan && (row + i) < heights.length; i++) {
                if ((row + i) == heights.length - 1) {
                    cellHeight = size.y - border - cellY - marginTop
                        - marginBottom - 1;
                } else {
                    cellHeight += (heights[row + i] == -1) ? grabHeight
                        : heights[row + i];
                }
                if (i > 0) {
