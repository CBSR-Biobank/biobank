...
package edu.ualberta.med.biobank.gui.common.widgets.nebula.tablecombo;
...
public class TableCombo extends Composite {
    ...
    public TableCombo(Composite parent, int style) {
        ...
        selectedImage.setBackground(Display.getCurrent().getSystemColor(
            SWT.COLOR_WHITE));
        ...
        int[] textEvents = { SWT.DefaultSelection, SWT.KeyDown, SWT.KeyUp,
            SWT.MenuDetect, SWT.Modify, SWT.MouseDown, SWT.MouseUp,
            SWT.MouseDoubleClick, SWT.MouseWheel, SWT.Traverse, SWT.FocusIn,
            SWT.Verify };
     ...
     * 
     * @param listener the listener which should be notified when the user
     *            presses keys in the text control.
     * 
     * @exception IllegalArgumentException <ul>
     *                <li>ERROR_NULL_ARGUMENT - if the listener is null</li>
     *                </ul>
    ...
    public Point computeSize(int wHint, int hHint, boolean changed) {
        ...
        if ((wHint != SWT.DEFAULT) && (hHint != SWT.DEFAULT)) {
            ...
            for (int i = 0; i < tableItems.length; i++) {
                currTextWidth = gc
                    .stringExtent(tableItems[i].getText(colIndex)).x;
                ...
                if (tableItems[i].getImage() != null) {
                    ...
                    maxImageHeight = Math.max(tableItems[i].getImage()
                        .getBounds().height, maxImageHeight);
                }
            ...
            Point arrowSize = arrow.computeSize(SWT.DEFAULT, SWT.DEFAULT,
                changed);
    ...
    void dropDown(boolean drop) {
        ...
        autoAdjustColumnWidthsIfNeeded(tableColumns, tableWidth,
            totalColumnWidth);
    ...
    public boolean isFocusControl() {
        ...
        if (arrow.isFocusControl() || table.isFocusControl()
    ...
    private void internalLayout(boolean changed) {
        ...
        if (selectedImage.getImage() == null) {
            ...
            Point imageSize = selectedImage.computeSize(SWT.DEFAULT, height,
                changed);
            ...
            arrow
                .setBounds(imageWidth + textWidth, 0, arrowSize.x, arrowSize.y);
        }
    }
    ...
    private void popupEvent(Event event) {
        ...
        case SWT.Paint:
            ...
            Rectangle tableRect = table.getBounds();
            event.gc.setForeground(getDisplay().getSystemColor(
                SWT.COLOR_LIST_SELECTION));
            event.gc.drawRectangle(0, 0, tableRect.width + 1,
                tableRect.height + 1);
            break;
        case SWT.Close:
            event.doit = false;
     ...
     * @param columnBounds
     */
    private void defineColumnsInternal(String[] columnHeaders,
        int[] columnBounds, int totalColumnsToBeCreated) {

        checkWidget();

        int totalColumnHeaders = columnHeaders == null ? 0
    ...
    private int getDisplayColumnIndex() {
        ...
        return (displayColumnIndex <= (table.getColumnCount() - 1) ? displayColumnIndex
            : 0);
    }
    ...
    private void autoAdjustColumnWidthsIfNeeded(TableColumn[] tableColumns,
        ...
        if (table.getVerticalBar().getVisible()) {
            scrollBarSize = (table.getVerticalBar() == null ? 0 : table
        ...
        if (totalAvailWidth > (totalColumnWidthUsage + scrollBarSize)) {
            ...
            if (totalAmtToBeAllocated > 0) {
                tableColumns[totalColumns - 1]
                    .setWidth(tableColumns[totalColumns - 1].getWidth()
