/*******************************************************************************
 * This code borrowed from the Eclipse JFace Data Binding examples.
 * 
...
import edu.ualberta.med.biobank.gui.common.widgets.utils.DuckType;
 ...
 * <p>
 * Its API is basically the same API as List, but with unnecessary methods removed.
 */
public class RadioGroup {
    ...
    /**
     * Constructs an instance of this widget given an array of Button objects to wrap. The Button
     * objects must have been created with the SWT.RADIO style bit set, and they must all be in the
     * same Composite.
     * 
    ...
    /**
     * Returns the object corresponding to the currently-selected radio button or null if no radio
     * button is selected.
     * 
    ...
    private final List<VetoableSelectionListener> widgetChangeListeners =
        new ArrayList<VetoableSelectionListener>();
    ...
    private final List<SelectionListener> widgetSelectedListeners = new ArrayList<SelectionListener>();
    ...
    /**
     * Deselects the item at the given zero-relative index in the receiver. If the item at the index
     * was already deselected, it remains deselected. Indices that are out of range are ignored.
     * 
    ...
    /**
     * Returns the zero-relative index of the item which currently has the focus in the receiver, or
     * -1 if no item has focus.
     * 
    ...
    public String getItem(int index) {
        if (index < 0 || index >= buttons.length)
            SWT.error(SWT.ERROR_INVALID_RANGE, null, "getItem for a nnexistant item");
    ...
    /**
     * Returns a (possibly empty) array of <code>String</code>s which are the items in the receiver.
     * <p>
    ...
    /**
     * Returns <code>true</code> if the item is selected, and <code>false</code> otherwise. Indices
