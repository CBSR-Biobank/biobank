...
public enum CellStatus {
    ...
    IN_SHIPMENT_EXPECTED(SWT.COLOR_DARK_GREEN, "Expected"), IN_SHIPMENT_ADDED(
        SWT.COLOR_DARK_GREEN, "Added"), IN_SHIPMENT_RECEIVED(
        SWT.COLOR_DARK_GRAY, "Received"), NOT_IN_SHIPMENT(SWT.COLOR_YELLOW,
        "Not in Shipment"), EXTRA(217, 161, 65, "Extra"), SCAN_PROFILE(185,
    ...
    public static List<CellStatus> DEFAULT_PALLET_SCAN_ASSIGN_STATUS_LIST =
        Arrays.asList(EMPTY, NEW, MOVED, FILLED, MISSING, ERROR);

    public static List<CellStatus> DEFAULT_PALLET_SCAN_LINK_STATUS_LIST =
        Arrays.asList(EMPTY, SCAN_PROFILE, NO_TYPE, TYPE, ERROR);

    public static List<CellStatus> DEFAULT_PALLET_DISPATCH_RECEIVE_STATUS_LIST =
        Arrays.asList(EMPTY, IN_SHIPMENT_RECEIVED, IN_SHIPMENT_EXPECTED, EXTRA,
            NOT_IN_SHIPMENT, ERROR);

    public static List<CellStatus> DEFAULT_PALLET_DISPATCH_CREATE_STATUS_LIST =
        Arrays.asList(EMPTY, FILLED, IN_SHIPMENT_ADDED, MISSING, ERROR);

    private CellStatus(int color, String legend) {
        this.color =
            BioBankPlugin.getDefault().getWorkbench().getDisplay()
    ...
    private CellStatus(int red, int green, int blue, String legend) {
        this.color =
            new Color(BioBankPlugin.getDefault().getWorkbench().getDisplay(),
                red, green, blue);
        this.legend = legend;
    }
