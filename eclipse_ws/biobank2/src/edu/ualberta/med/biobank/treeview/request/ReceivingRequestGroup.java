...
public class ReceivingRequestGroup extends AbstractRequestGroup {
    private static final I18n i18n = I18nFactory.getI18n(ReceivingRequestGroup.class);

    @SuppressWarnings("nls")
    public ReceivingRequestGroup(AdapterBase parent, int id) {
        super(parent, id, i18n.tr("Pending Requests"));

        this.isReadable = isAllowed(new RequestReadPermission());
    }

    @Override
    protected List<? extends ModelWrapper<?>> getWrapperChildren() throws Exception {
        if (!isReadable) {
            // user does not have permission to read requests, return empty list
