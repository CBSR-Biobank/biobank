...
public class SharedReportsGroup extends AbstractReportGroup {
    private static final String NODE_NAME = "Shared Reports";
    ...
    private static final String HQL_REPORT_OF_USER = "from "
        ...
        + USER_ID_LIST_TOKEN + ")";
    private static final String SQL_USERS_IN_SAME_GROUP = "SELECT CONVERT(u2.user_id, CHAR) FROM csm_user u1, csm_user u2 WHERE u1.user_id = "
        + USER_ID_TOKEN
        + " AND NOT EXISTS (SELECT g1.group_id FROM csm_user_group g1 WHERE g1.user_id = u1.user_id AND g1.group_id NOT IN (SELECT g2.group_id FROM csm_user_group g2 WHERE g2.user_id = u2.user_id)) AND NOT EXISTS (SELECT g1.group_id FROM csm_user_group g1 WHERE g1.user_id = u2.user_id AND g1.group_id NOT IN (SELECT g2.group_id FROM csm_user_group g2 WHERE g2.user_id = u1.user_id))";
