...
import edu.ualberta.med.biobank.widgets.nebula.v.VButtonPainter;
import edu.ualberta.med.biobank.widgets.nebula.v.VControl;
import edu.ualberta.med.biobank.widgets.nebula.v.VLabelPainter;
import edu.ualberta.med.biobank.widgets.nebula.v.VPanel;
import edu.ualberta.med.biobank.widgets.nebula.v.VPanelPainter;
        ...
        if (picker instanceof DatePicker) {
            ...
            for (int i = 1; i < days.length; i++) {
                ...
                if (cal.get(Calendar.DAY_OF_MONTH) == 1 && !isActive(day)
                    ...
                    if (indexOf(day) % 7 != 0) {
                        e.gc.drawLine(bounds.x, bounds.y, bounds.x, bounds.y
                            + bounds.height);
                    }
                    if (indexOf(day) > 7) {
                        e.gc.drawLine(bounds.x, bounds.y, pbounds.x
                            + pbounds.width, bounds.y);
                    }
                    e.gc.drawLine(pbounds.x, bounds.y + bounds.height,
