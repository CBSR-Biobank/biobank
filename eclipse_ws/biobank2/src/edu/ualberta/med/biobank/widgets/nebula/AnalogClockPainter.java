...
class AnalogClockPainter implements IControlPainter {
    ...
    public final void paintContent(VControl control, Event e) {
        ...
        if (picker.secHand) {
            ...
            if (picker.overSec) {
                ...
                e.gc
                    .setForeground(e.display.getSystemColor(SWT.COLOR_DARK_RED));
                e.gc.setLineWidth(2);
            }
    ...
    protected void paintFace(VPanel panel, Event e) {
        ...
        Pattern p = new Pattern(e.display, 0, y, 0, y + dia, e.display
            .getSystemColor(SWT.COLOR_WHITE), c1);
        ...
        for (int i = 0; i < inc; i++) {
            int x2 = x
                + picker.dialRadius
                + (int) ((picker.dialRadius - 25) * (Math.cos(2 * (double) i
                    * Math.PI / inc - Math.PI / 2)));
            int y2 = y
                + picker.dialRadius
                + (int) ((picker.dialRadius - 25) * (Math.sin(2 * (double) i
                    * Math.PI / inc - Math.PI / 2)));
