...
import edu.ualberta.med.biobank.widgets.nebula.v.VButton;
...
class AnalogTimePicker extends VPanel {
    ...
    class BaseLayout extends VLayout {
        ...
        protected void layout(VPanel panel, boolean flushCache) {
            ...
            Point dclockSize = (digitalClock != null) ? digitalClock
                .computeSize(SWT.DEFAULT, SWT.DEFAULT, flushCache) : new Point(
                0, 0);
            ...
            if (digitalClock != null) {
                digitalClock.setBounds(clientArea.x
                    + (clientArea.width - dclockSize.x) / 2, dy + dheight,
                    dclockSize.x, dclockSize.y);
            }
        }
    }
    ...
    class DialLayout extends VLayout {
        ...
        protected void layout(VPanel panel, boolean flushCache) {
            ...
            if (timeAmPm != null) {
                ...
                timeAmPm.setBounds(dialCenter.x - (size.x / 2), dialCenter.y
                    + (dialRadius / 3) - (size.y / 4), size.x, size.y);
            }
        }
    }
    ...
    public int[] getFields() {
        return new int[] { Calendar.HOUR_OF_DAY, Calendar.HOUR,
            Calendar.MINUTE, Calendar.SECOND, Calendar.AM_PM };
    }
    ...
    private void handleMouseMove(int x, int y) {
        ...
        if (setH) {
            ...
            if (Math.sqrt(dx * dx + dy * dy) < dialRadius) {
                double h = (tmpcal.get(is24Hour ? Calendar.HOUR_OF_DAY
                    : Calendar.HOUR) + (double) tmpcal.get(Calendar.MINUTE) / 60)
                    * (is24Hour ? 2.5 : 5);
    ...
    void updateLabels() {
        if (timeNow != null) {
            timeNow.setToolTipText(Resources.getString(
                "nav_current_time", cdt.getLocale())); //$NON-NLS-1$
        }
    }
