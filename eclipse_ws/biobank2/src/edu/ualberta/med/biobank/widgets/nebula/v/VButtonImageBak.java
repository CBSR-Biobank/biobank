...
class VButtonImageBak {
    ...
    private class ImageListener implements Listener {
        ...
        public void handleEvent(Event e) {
            ...
            if (requests.containsKey(key)) {
                for (Iterator<VButton> iter = requests.get(key).iterator(); iter
                    .hasNext();) {
                    iter.next().redraw();
                    iter.remove();
                }
    ...
    private void createImage(VButton button) {
        ...
        if (requests.containsKey(key)) {
            ...
            if (style == 0) {
                style = SWT.TOGGLE; // defaults to, and converts PUSH buttons
                                    // to, TOGGLE
            }
    ...
    void removeUnusedData() {
        for (Iterator<ImageData> i1 = buttons.keySet().iterator(); i1.hasNext();) {
            ...
            for (Iterator<VButton> i2 = buttons.get(data).iterator(); i2
                .hasNext();) {
