...
class SvgGradient extends SvgElement {
    ...
    public void create(SvgShape shape, GC gc) {
        ...
        try {
            ...
            // XXX fix reference to org.eclipse.nebula
            ...
            System.out
                .println("Could not create pattern - fragment must be missing"); //$NON-NLS-1$
        }
    }

    private Color createColor(GC gc, int color) {
        return new Color(gc.getDevice(), color >> 16, (color & 0x00FF00) >> 8,
            color & 0x0000FF);
    }
