...
public final class TestReportsProcedurallyGeneratedData implements
    ...
    private static final int NUM_CLINICS_PER_SITE = 7;
    private static final int NUM_CONTAINER_ROWS = 5;
    private static final int NUM_CONTAINER_COLS = 5;
    // the maximum ratio of "filled positions" / "total positions" for a
    // container; should be at least (int) 1 / (NUM_CONTAINER_ROWS *
    // NUM_CONTAINER_COLS)
    private static final double MAX_CONTAINER_CAPACITY = 0.1d;
    ...
    private static final int SHIPMENTS_PER_SITE = 43;
    ...
    // leave every xth aliquot empty
    private static final int SKIP_ALIQUOT = 10;
    ...
    private static List<ClinicWrapper> generateClinics(SiteWrapper site,
        final int numClinics) throws Exception {
        List<ClinicWrapper> clinics = new ArrayList<ClinicWrapper>();
        for (int i = 0; i < numClinics; i++) {
            clinics.add(ClinicHelper.addClinic(site, getInstance()
                .getRandString()));
        }
        return clinics;
    }
    ...
    private static List<ShipmentWrapper> generateShipments(SiteWrapper site,
        final int shipmentLimit, List<ClinicWrapper> clinics)
            ...
            while ((shipmentsAdded < shipmentLimit)
                ...
                if (patientLimit > 0) {
                    ShipmentWrapper shipment = ShipmentHelper.addShipment(site,
                        clinic, patients.get(patientIndex));
    ...
    public static void setUp() throws Exception {
        ...
        for (SiteWrapper site : sites) {
            List<ClinicWrapper> clinics = generateClinics(site,
                NUM_CLINICS_PER_SITE);
