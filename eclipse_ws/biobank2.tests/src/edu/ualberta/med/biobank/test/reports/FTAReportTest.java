...
public class FTAReportTest extends AbstractReportTest {
    private static final Predicate<AliquotWrapper> ALIQUOT_FTA_SAMPLE_TYPE =
        new Predicate<AliquotWrapper>() {
    ...
    private static final Mapper<PatientVisitWrapper, String, PatientVisitWrapper> GROUP_PATIENT_VISITS_BY_PNUMBER =
        new Mapper<PatientVisitWrapper, String, PatientVisitWrapper>() {
            public String getKey(PatientVisitWrapper patientVisit) {
                return patientVisit.getPatient().getPnumber();
            }

            public PatientVisitWrapper getValue(
                PatientVisitWrapper patientVisit, PatientVisitWrapper oldValue) {
    ...
    private static final Mapper<AliquotWrapper, String, AliquotWrapper> GROUP_ALIQUOTS_BY_PNUMBER =
        new Mapper<AliquotWrapper, String, AliquotWrapper>() {
    ...
    @Deprecated
    private static final Mapper<AliquotWrapper, String, AliquotWrapper> GROUP_ALIQUOTS_BY_PNUMBER_OLD =
        new Mapper<AliquotWrapper, String, AliquotWrapper>() {
            public String getKey(AliquotWrapper aliquot) {
                return aliquot.getPatientVisit().getPatient().getPnumber();
            }
    ...
    protected Collection<Object> getExpectedResults() throws Exception {
        ...
        Predicate<PatientVisitWrapper> patientInStudy =
            new Predicate<PatientVisitWrapper>() {
        ...
        Predicate<AliquotWrapper> pvProcessedAfter =
            new Predicate<AliquotWrapper>() {
                public boolean evaluate(AliquotWrapper aliquot) {
        ...
        Collection<PatientVisitWrapper> filteredPatientVisits =
            PredicateUtil.filter(allPatientVisits, patientInStudy);
        Map<String, PatientVisitWrapper> groupedPatientVisits =
            MapperUtil.map(filteredPatientVisits,
                GROUP_PATIENT_VISITS_BY_PNUMBER);

        Collection<AliquotWrapper> allAliquots = getAliquots();
        @SuppressWarnings("unchecked")
        Collection<AliquotWrapper> filteredAliquots =
            PredicateUtil.filter(allAliquots, PredicateUtil
                .andPredicate(ALIQUOT_FTA_SAMPLE_TYPE, pvProcessedAfter,
                    ALIQUOT_HAS_POSITION));
        Map<String, AliquotWrapper> groupedAliquots =
            MapperUtil.map(filteredAliquots, GROUP_ALIQUOTS_BY_PNUMBER);
        List<AliquotWrapper> filteredAndGroupedAliquots =
            new ArrayList<AliquotWrapper>(groupedAliquots.values());
