...
public class CollectionEventWrapper extends
    ...
    private void checkNoMoreProcessingEvents() throws BiobankCheckException {
        List<ProcessingEventWrapper> visits = getPatientVisitCollection();
    ...
    public boolean hasVisitForPatient(PatientWrapper patient) {
        List<ProcessingEventWrapper> pvs = patient.getPatientVisitCollection();
        if (pvs != null)
    ...
    public List<ProcessingEventWrapper> getPatientVisitCollection() {
        List<ProcessingEventWrapper> patientVisitCollection = new ArrayList<ProcessingEventWrapper>();
        Collection<ShipmentPatientWrapper> csps = getShipmentPatientCollection();
        if (csps != null) {
            for (ShipmentPatientWrapper csp : csps) {
                for (ProcessingEventWrapper visit : csp
    ...
    public void addPatientVisits(List<ProcessingEventWrapper> newPatientVisits)
        throws BiobankCheckException {
        if (newPatientVisits != null && newPatientVisits.size() > 0) {
            Collection<PatientVisit> allVisitObjects = new HashSet<PatientVisit>();
            List<ProcessingEventWrapper> allVisitWrappers = new ArrayList<ProcessingEventWrapper>();
            // already added visits
            List<ProcessingEventWrapper> currentList = getPatientVisitCollection();
            Collection<PatientVisit> oldCollection = new ArrayList<PatientVisit>();
            for (ProcessingEventWrapper visit : currentList) {
            ...
            // new
            Collection<ShipmentPatientWrapper> csps = getShipmentPatientCollection();
            Collection<ProcessingEventWrapper> pvs;
            for (ProcessingEventWrapper newVisit : newPatientVisits) {
    ...
    public static List<CollectionEventWrapper> getShipmentsInSites(
        ...
        List<CollectionEventWrapper> wrappers = new ArrayList<CollectionEventWrapper>();
        for (Shipment s : shipments) {
            wrappers.add(new CollectionEventWrapper(appService, s));
        }
        return wrappers;
    }
    ...
    public static List<CollectionEventWrapper> getTodayShipments(
        ...
        List<CollectionEventWrapper> ships = new ArrayList<CollectionEventWrapper>();
        for (Shipment s : res) {
            ships.add(new CollectionEventWrapper(appService, s));
        }
        return ships;
    }
