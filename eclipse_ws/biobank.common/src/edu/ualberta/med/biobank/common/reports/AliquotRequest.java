    ...
    public AliquotRequest(String op, Integer siteId) {
        super(
            ...
            "select s.patientVisit.patient.study.nameShort, s.inventoryId, s.patientVisit.dateDrawn, s.sampleType.name, s.aliquotPosition.container.label from "
                ...
                + op
                + siteId
                + " and Alias.aliquotPosition.container.label not like 'SS%' and s.patientVisit.patient.study.nameShort like ? and datediff(s.linkDate, ?) between 0 and 1  and s.sampleType.name like ? ORDER BY RAND()",
