...
public class SpecimenMicroplateConsistentAction implements Action<BooleanResult> {
    ...
    private static final long serialVersionUID = 1L;
    private Integer centerId; // center where new or all specimens in list will be located
    private Boolean justNew; // centerId is just for new specimens in list, as opposed to all specimens in list
    private List<SpecimenMicroplateInfo> specimenMicroplateInfos; // (potentially microplate) specimens to be created, changed, moved or stored

    public SpecimenMicroplateConsistentAction (Integer centerId, Boolean justNew, List<SpecimenMicroplateInfo> specimenMicroplateInfos) {
    ...
    public BooleanResult run(ActionContext context) throws ActionException {
    	...
    	if (microplateIds.size() > 0) {
            ...
            for (String mpId : microplateIds) {
                ...
                if (justNew) {
                	// new specimens in list will be at centerId, but others for same microplate are elsewhere
                	...
                	// all specimens in list will be at centerId, but others for same microplate are elsewhere
        	...
        	if (containerId == null) {
        		for (String mpId : microplateIds) {
                    ...
                    // all specimens in list will not be in any container, but other(s) for same microplate are in some container(s) 
                    if (!inventoryIds.containsAll(query.list())) {
                        throw new ActionException("Specimens for microplate " //$NON-NLS-1$
        	...
        	if (containerId != null) {
                ...
                // all specimens in list will be in containerId, but others for same microplate are stored elsewhere or not stored in any container
                if (!inventoryIds.containsAll(query.list())) {
                    throw new ActionException("Specimens for microplate " //$NON-NLS-1$
