...
public class NewPsByStudyClinicMonth extends QueryObject {
    ...
    public NewPsByStudyClinicMonth(String op, Integer siteId) {
        super(
            "Displays the total number of patients added per study per clinic by month.",
            "Select ( "
                + MessageFormat.format(sub_query, op, siteId)
                + ") as dateProcessed, Alias.patient.study.name, Alias.shipment.clinic.name from  "
                + PatientVisit.class.getName()
                + " as Alias where Alias.patient.study.site "
                + op
                + siteId
                + "ORDER BY Alias.patient.study.name, Alias.shipment.clinic.name, dateProcessed ASC",
