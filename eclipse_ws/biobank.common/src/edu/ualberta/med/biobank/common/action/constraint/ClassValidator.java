...
public class ClassValidator<T> {
    ...
    private static final Logger log = LoggerFactory
        .getLogger(ClassValidator.class);
    private static final InvalidValue[] EMPTY_INVALID_VALUE_ARRAY =
        new InvalidValue[] {};
    ...
    public ClassValidator(Class<T> beanClass, ResourceBundle resourceBundle) {
        this(beanClass, resourceBundle, null,
            new HashMap<XClass, ClassValidator>(), null);
    }
    ...
    public ClassValidator(Class<T> beanClass, MessageInterpolator interpolator) {
        this(beanClass, null, interpolator,
            new HashMap<XClass, ClassValidator>(), null);
    }
    ...
    public ClassValidator(
        ...
        this.reflectionManager =
            reflectionManager != null ? reflectionManager
        ...
        this.messageBundle = resourceBundle == null ?
            ...
            resourceBundle;
        this.defaultMessageBundle =
            ResourceBundle.getBundle(DEFAULT_VALIDATOR_MESSAGE);
    ...
    protected ClassValidator(
        XClass beanXClass, ResourceBundle resourceBundle,
    ...
    private ResourceBundle getDefaultResourceBundle() {
        ...
        try {
            ...
            log.trace(
                "ResourceBundle {} not found in thread context classloader",
                VALIDATOR_MESSAGE);
    ...
    private void addSuperClassesAndInterfaces(XClass clazz,
        Collection<XClass> classes) {
    ...
    private boolean handleAggregateAnnotations(Annotation annotation,
        XMember member) {
        Object[] values;
        try {
    ...
    private void createChildValidator(XMember member) {
        if (member.isAnnotationPresent(Valid.class)) {
            ...
            if (!childClassValidators.containsKey(clazz)) {
                ...
                new ClassValidator(clazz, messageBundle, userInterpolator,
                    childClassValidators, reflectionManager);
            }
        }
    }
    ...
    private void createMemberValidator(XMember member) {
        ...
        for (Annotation methodAnnotation : memberAnnotations) {
            ...
            boolean agrValidPresent =
                handleAggregateAnnotations(methodAnnotation, member);
            validatorPresent = validatorPresent || agrValidPresent;
        }
        if (validatorPresent && !member.isTypeResolved()) {
            log.warn(
                "Original type of property {} is unbound and has been approximated.",
                member);
        }
    }
    ...
    private Validator createValidator(Annotation annotation) {
        try {
            ValidatorClass validatorClass =
                annotation.annotationType().getAnnotation(ValidatorClass.class);
                    ...
                    if (getter.isCollection()) {
                        ...
                        for (Object element : elements) {
                            Object actualElement =
                                isIterable ? element : map.get(element);
                            if (actualElement == null) {
                                index++;
                                continue;
                            }
                            InvalidValue[] invalidValues =
                                getClassValidator(actualElement)
                                    .getInvalidValues(actualElement,
                                        circularityState);
                            ...
                            for (InvalidValue invalidValue : invalidValues) {
                                invalidValue.addParentBean(bean,
                                    indexedPropName);
    ...
    private ClassValidator getClassValidator(Object value) {
        ...
        ClassValidator validator =
            childClassValidators.get(reflectionManager.toXClass(clazz));
    ...
    private String getPropertyName(XMember member) {
        ...
        if (XProperty.class.isAssignableFrom(member.getClass())) {
            ...
            if (propertyName.startsWith("is")) {
                propertyName =
                    Introspector.decapitalize(propertyName.substring(2));
            }
