...
$queries['collectionEvents'] = <<<QRY_CEVENTS
...
$queries['collectionEventComments'] = <<<QRY_CEVENT_COMMENTS
  SELECT study.name_short, patient.pnumber, ce.visit_number,
  cmt.message comment, cmt.created_at comment_date
  FROM collection_event ce
  JOIN patient on patient.id=ce.patient_id
  JOIN study on study.id=patient.study_id
...
$queries['specimens'] = <<<QRY_SPECIMENS
  SELECT study.name_short study, pt.pnumber patient_number, ce.visit_number, spc.inventory_id, stype.name,
         pe.worksheet source_worksheet,
         pspc.inventory_id parent_inventory_id,
         spc.created_at, pspc.created_at time_drawn, spc.quantity,
