...
INSERT INTO csm_user (USER_ID, LOGIN_NAME, MIGRATED_FLAG, FIRST_NAME, LAST_NAME, PASSWORD, UPDATE_DATE)
       select coalesce(MAX(id), 0)+Max(user_id)+1, 'testprocessor', 0 , 'testprocessor', 'testprocessor', 'orDBlaojDQE=', sysdate()
...
insert into principal (id, version, discriminator, login, csm_user_id, recv_bulk_emails, full_name, email, need_pwd_change,activity_status_id)
       ...
       from csm_user
       where login_name = 'testprocessor';
...
-- add limited access group 'Technician Level 2'
insert into principal values ('BbGroup', 158, 0, 2, NULL, 'Technician Level 2', NULL, NULL, NULL, NULL, NULL, NULL);	   
	   
-- add testuser to group 'Technician Level 2'
insert into group_user(user_id, group_id)
       select u.id, g.id
       from principal u, principal g
       where u.login='testprocessor'
       and g.name='Technician Level 2';

-- add a Technician Level 2 membership for site CBSR (id = 34)
insert into membership (id, version, center_id, NOT_NULL_CENTER_ID, principal_id,rank,level)
       select coalesce(MAX(ms.id), 0)+1, 0, 34, 34, 158,2,999
