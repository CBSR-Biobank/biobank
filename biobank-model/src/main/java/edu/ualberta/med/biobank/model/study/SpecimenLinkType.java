...
@Audited
@Entity
@Table(name = "SPECIMEN_LINK_TYPE", uniqueConstraints = {
...
@NotUsed(by = SpecimenLink.class, property = "type", groups = PreDelete.class)
public class SpecimenLinkType
    ...
    /**
     * @return the {@link ProcessingType} that this {@link SpecimenLinkType}
     *         belongs to, which contains additional textual information and
     *         descriptions.
     */
    @NotNull(message = "{SpecimenLinkType.type.NotNull}")
     ...
     */
    @NotNull(message = "{SpecimenLinkType.inputGroup.NotNull}")
    @ManyToOne(fetch = FetchType.LAZY)
     ...
     */
    @NotNull(message = "{SpecimenLinkType.outputGroup.NotNull}")
    @ManyToOne(fetch = FetchType.LAZY)
